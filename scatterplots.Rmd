...
# Two-Sample Test of Proportions: AZ vs WV

```{r prop_test}

az_wv_df = brfss_epa_df %>%
  filter(state %in% c("AZ", "WV")) %>%
  group_by(state) %>%
  mutate(
...
prop_results = prop.test(x = az_wv_df$current_asthma, 
          n = az_wv_df$n, 
...
knitr::kable(prop_results, 
             digits = 2,
             caption = "*Two-Sample Test of Proportions: AZ vs WV*")
